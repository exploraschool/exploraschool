...
export function AccountWelcomeWizard({ locale, initialProfile }: AccountWelcomeWizardProps) {
  const router = useRouter();
  const t = useTranslations("account");
  const [step, setStep] = useState<Step>(initialProfile?.hasTakenClassesBefore == null ? "ask" : "wizard");
      ...
      {step === "ask" ? (
        ...
        <div className="rounded-3xl border border-hielo/10 bg-white p-6 shadow-[0_16px_50px_rgba(14,14,15,0.06)] sm:p-8">
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-oro">
            {t("wizardStep", { current: wizardStep, total: 4 })}
          </p>
          <h1 className="mt-2 font-display text-3xl font-semibold text-hielo">{t(`wizardTitle${wizardStep}`)}</h1>
          <p className="mt-3 text-sm text-muted">{t(`wizardLead${wizardStep}`)}</p>
            ...
            {wizardStep === 3 ? (
              <div className="space-y-3">
                {companions.map((companion, index) => (
                  <div key={companion.id} className="grid gap-2 rounded-2xl border border-hielo/10 bg-nieve p-3 sm:grid-cols-[1fr_8rem_5rem_auto]">
                    <input
                    ...
                    <button
                      ...
                      onClick={() => setCompanions((current) => current.filter((item) => item.id !== companion.id))}
                    >
                      {t("remove")}
                    </button>
                  </div>
          ...
          <div className="mt-8 flex justify-between gap-3">
            ...
            {wizardStep < 4 ? (
              ...
              <button type="button" className="btn-primary !w-auto" disabled={!canNext || busy} onClick={() => void onFinish()}>
                {t("finish")}
              </button>
            )}
          </div>
