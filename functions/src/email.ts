...
export function buildCustomerBookingReceivedEmail(params: {
  data: Record<string, unknown>;
  siteUrl: string;
}): { subject: string; text: string; html: string } {
  const { data, siteUrl } = params;
    ...
    .map((item, index) => {
      ...
      return [
        ...
        item.timeSlotLabel
          ...
          : "",
        item.participants ? `   ${pick(isEn, "Personas", "People")}: ${item.participants}` : "",
        item.instructorName
          ? `   ${pick(isEn, "Monitor/a preferido/a", "Preferred instructor")}: ${item.instructorName}`
          : "",
      ]
      ...
      <td align="center" style="padding:28px 16px;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background:${BRAND.white};border:1px solid ${BRAND.border};border-radius:20px;overflow:hidden;">
          <tr>
            <td style="background:${BRAND.nieve};padding:28px 28px 18px;text-align:center;border-bottom:1px solid ${BRAND.border};">
              <img src="${escapeHtml(logoUrl)}" width="88" height="88" alt="Explora School & Club" style="display:block;margin:0 auto 14px;border:0;outline:none;" />
              <p style="margin:0;font-family:Georgia,'Times New Roman',serif;font-size:26px;line-height:1.25;color:${BRAND.pizarra};font-weight:700;">${escapeHtml(pick(isEn, "Solicitud recibida", "Request received"))}</p>
              <p style="margin:8px 0 0;font-family:Arial,Helvetica,sans-serif;font-size:13px;letter-spacing:0.08em;text-transform:uppercase;color:${BRAND.hielo};font-weight:700;">Explora School &amp; Club</p>
            </td>
          </tr>
          <tr>
            <td style="height:4px;background:${BRAND.hielo};font-size:0;line-height:0;">&nbsp;</td>
          ...
          <tr>
            <td style="padding:28px;font-family:Arial,Helvetica,sans-serif;">
              ...
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;background:${BRAND.nieve};border:1px solid ${BRAND.border};border-radius:12px;">
                <tr>
                  <td style="padding:14px 16px;font-size:14px;line-height:1.55;color:${BRAND.pizarra};">${escapeHtml(notConfirmed)}</td>
                </tr>
              </table>
              ${
                items.length > 0
              ...
              ${
                estimatedTotal !== undefined
                  ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:16px 0 0;border-top:1px solid ${BRAND.border};">
                      ...
                      <tr>
                        <td colspan="2" style="padding:0 0 4px;font-size:13px;line-height:1.5;color:${BRAND.muted};">${escapeHtml(totalNote)}</td>
                      </tr>
                    </table>`
                  : ""
              }
              <p style="margin:28px 0 12px;font-size:15px;line-height:1.55;color:${BRAND.pizarra};">${escapeHtml(pick(isEn, "Si necesitas añadir algún detalle, responde a este correo o escríbenos por WhatsApp.", "If you need to add any details, reply to this email or write to us on WhatsApp."))}</p>
              <table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 0 8px;">
                <tr>
                  <td style="border-radius:999px;background:${BRAND.accent};">
                    <a href="${escapeHtml(CONTACT.whatsappUrl)}" style="display:inline-block;padding:12px 22px;font-family:Arial,Helvetica,sans-serif;font-size:14px;font-weight:700;color:${BRAND.white};text-decoration:none;">${escapeHtml(pick(isEn, "WhatsApp Explora", "WhatsApp Explora"))}</a>
